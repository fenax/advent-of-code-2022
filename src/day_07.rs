...
impl Item{
    ...
    pub fn size(&self)->usize{
        match self{
            Item::Folder(_, content)=>
                content.iter().map(Item::size).sum(),
...
fn part_2(data: &Data)->Int{
    ...
    data.run_on_all(&|item, (needed, closest)|{
        if let Item::Folder(_, _) = item {
            let size = item.size(); 
...
mod tests {
    ...
    use crate::day_07::{FILE,parse_input, part_1, part_2, Item};
...
";
    ...
    fn test_example() {
        let exemple =
        Item::Folder( "/".to_string(), vec![
            Item::Folder("a".to_string(),vec![
                Item::Folder("e".to_string(),vec![
                    Item::File("i".to_string(),584)]),
