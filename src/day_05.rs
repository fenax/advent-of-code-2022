...
fn parse_input(input:&str) -> Data{
    ...
    let columns : Vec<_> = stacks.pop().unwrap().chars().enumerate().filter_map(
        |(i,c)|{
            if c.is_whitespace(){
                None
            }else{
    ...
    (
        ...
        v[1].lines().map(
            |l|{
                ...
                (words[1].parse().unwrap(),words[3].parse().unwrap(),words[5].parse().unwrap())
            }).collect()
    )
}
...
fn part_1(data: &Data)->Int{
    ...
    stacks.iter_mut().map(Vec::pop).map(|x|x.unwrap_or(' ')).collect()
}
...
mod tests {
    ...
    use crate::day_05::{FILE,parse_input, part_1, part_2};
