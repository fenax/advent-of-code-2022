...
fn part_1(data: &Data)->Int{
    ...
    for line in 0..data.height{
        do_line(data,line_offset*line..line_offset*line+data.width,&mut visible_map);
        do_line(data,(line_offset*line..line_offset*line+data.width).rev(),&mut visible_map);
    }
    for column in 0..data.width{
        do_line(data, (column..column+(line_offset*data.height)).step_by(line_offset),&mut visible_map);
        do_line(data, (column..column+(line_offset*data.height)).step_by(line_offset).rev(),&mut visible_map);
    }
...
fn part_2(data: &Data)->Int{
    ...
    for line in 0..data.height{
        for column in 0..data.width{
            ...
            let down = do_line(init,data, indexer(cell_index, line_offset as isize,limit).take(cells_down));
            let up = do_line(init,data, indexer(cell_index,-(line_offset as isize),limit).take(cells_up));
...
mod tests {
    ...
    use crate::day_08::{FILE,parse_input, part_1, part_2};
