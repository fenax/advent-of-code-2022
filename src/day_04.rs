...
fn parse_input(input:&str) -> Data{
    input
        ...
        .map(
            |x|{
                let mut x= x.split(|x| x == '-' || x == ',').map(str::parse).map(Result::unwrap);
...
fn part_1(data: &Data)->Int{
    data.iter().filter(|((a,b),(x,y))| {(*a>=*x && *y>=*b) ||(*x>=*a&& *b>=*y)}).count() as Int
}
...
mod tests {
    ...
    use crate::day_04::{FILE,parse_input, part_1, part_2};
