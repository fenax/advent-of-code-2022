...
fn part_1(data: &Data)->Int{
    data.iter().map(
        |(a,b)|{
            priority_from_mask(a&b)
        }
    ).sum::<u32>() as Int
}

fn part_2(data: &Data)->Int{
    data.windows(3).step_by(3).map(
        |group|{
            group.iter().fold(!0u64,|acc,(a,b)|{ acc & (*a|*b)})
        }
    ).map(priority_from_mask).sum::<u32>() as Int
}

fn slice_to_mask(data:&[u8])->u64{
    data.iter().map(to_priority_mask).fold(0u64,|acc, x| (acc | x))
}
...
mod tests {
    ...
    use crate::day_03::{FILE,parse_input, part_1, part_2};
