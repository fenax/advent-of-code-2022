...
fn round_score(other:Play, me:Play)->Int{
    let ret = (match (me, other){
        (Play::Rock,Play::Paper) | (Play::Paper, Play::Scissors) | (Play::Scissors, Play::Rock) => 0,
        (Play::Rock,Play::Rock) | (Play::Paper,Play::Paper) | (Play::Scissors, Play::Scissors) => 3,
        (Play::Paper,Play::Rock) | (Play::Scissors, Play::Paper) | (Play::Rock, Play::Scissors) => 6
    }) + (me as Int);
...
fn parse_input(input:&str) -> Data{
    input.split("\n").filter(|v|!v.is_empty()).map(|round|
        (round.as_bytes()[0],round.as_bytes()[2])
    ).collect()
}
...
mod tests {
    ...
    use crate::day_02::{FILE,calculate_score,calculate_score_2,parse_input};
